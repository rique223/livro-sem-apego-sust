<template lang="pug">
    ...
    form.form-styl( method = 'post' data-form="createAccount" enctype="multipart/form-data" @submit.prevent="validateForm" )
        ...
        label.field-box.-cep
            ...
            the-mask.field(v-validate="'required'" v-bind:class="{'-invalid': errors.has('cep')}" type = 'text'  name = 'cep' v-model = "cep" maxlength="10" :mask="'##.###-###'")/
...
export default {
  ...
  data() {
    return {
      ...
      previewImage: {
        ...
        title:
          "Foto de Perfil: <strong>(Tamanho Máx. 5MB e Extensão .png ou .jpg)</strong>",
        circle: true,
        src: false,
      },
    };
  ...
  methods: {
    ...
    verifyExistEmail() {
      ...
      this.$http.post(window.APIUrl + "/user/email", obj).then(
        (rs) => {
          rs.json().then(
            (rs) => {
